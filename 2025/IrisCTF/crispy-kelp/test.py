...
# c799c4a9c79dc493c5a2c7bbc5b3c698c6bac4bbc4acc597c480c981c981c981c981c982c982c982c982c983c983c983c983
left = 'c799c4a9c79dc493c5a2c7bbc5b3c698c6bac4bbc4acc597'
right = 'c981c981c981c981c982c982c982c982c983c983c983c983'

left = bytes.fromhex(left).decode('utf-8')
right = bytes.fromhex(right).decode('utf-8')

print(len(left), len(right))

# left = [int(left[i:i+6], 16) for i in range(0, len(left), 6)]
# right = [int(right[i:i+8], 16) for i in range(0, len(right), 8)]
...
for l, r in zip(left, right):
    print(l, r, ord(l), ord(r), ord(l) ^ ord(r))