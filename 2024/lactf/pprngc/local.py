...
f = lambda x: (x * 0x1233 + 0xb) & 0xffff
f_inverse = lambda x: (x - 0xb) * pow(0x1233, -1, 0x10000) & 0xffff
...
def prng(pred):
    ...
    cur_state = f(cur_state)
    cur_state_bits = format(cur_state, 'b').zfill(8)
...
if __name__ == "__main__":
    while not done:
        ...
        if choice == "1":
            rand_pred = secrets.randbelow(2**8)
